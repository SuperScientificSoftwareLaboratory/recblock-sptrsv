                                          ...
                                          int *d_levelItem)
{
    ...
    if (global_x_id < m)
    {
        ...
        const int perm_id = d_levelItem[rowidx];
                                    ...
                                    double        *preprocess)
{
    ...
    *preprocess += (t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0;
    ...
    cudaMemcpy(d_csrRowPtrTR, csrRowPtrTR_new, (m+1) * sizeof(int),   cudaMemcpyHostToDevice);
    cudaMemcpy(d_csrColIdxTR, csrColIdxTR_new, nnzTR  * sizeof(int),   cudaMemcpyHostToDevice);
    cudaMemcpy(d_csrValTR,    csrValTR_new,    nnzTR  * sizeof(VALUE_TYPE),   cudaMemcpyHostToDevice);
    ...
    *preprocess += time_cuda_analysis;
    ...
    {
        if (nlv > LEVELSET_NLEVEL_THRESHOLD)
        {
        ...
        if (rhs == 1)
        {
            ...
            else
            {
                ...
                {
                    //printf("SYNC-FREE (m = %i): else if (!fasttrack && nlv > LEVELSET_NLEVEL_THRESHOLD), nlevel = %i, nnzTR = %i, nnzr = %4.2f\n", 
                    //       m, nlv, nnzTR, (double)nnzTR/(double)m);
                    num_threads = WARP_PER_BLOCK * WARP_SIZE;
