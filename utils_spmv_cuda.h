                                  ...
                                  double        *preprocess)
{
    ...
    *preprocess += (t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0;
    ...
    int nnzr = (nnz-longlen) / m_new;
    double empty_ratio = 100*(double)(m-m_new)/(double)m;
    //printf("nnzr = %i, empty_ratio = %4.2f\n", nnzr, empty_ratio);
    ...
    cudaMemcpy(y, d_y, m * rhs * sizeof(VALUE_TYPE), cudaMemcpyDeviceToHost);
