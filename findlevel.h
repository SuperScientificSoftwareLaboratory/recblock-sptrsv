                        ...
                        int           *parallelism_max)
{
    ...
    //    {
    //        int idx = levelItem[j];
    //        levelnnz += cscColPtr[idx+1] - cscColPtr[idx];
    //    }
        //printf("level[%i] #cols = %i #nnz = %i\n", i, levelPtr[i+1] - levelPtr[i], levelnnz);
        //printf("%i\n", levelnnz - (levelPtr[i+1] - levelPtr[i]));
    //    allnnz += levelnnz;
    //}
    //printf("--------------\n");
    //printf ("allnnz in level-set is %i, nnz = %i. CHECK %s\n", 
    //        allnnz, nnz, allnnz == nnz ? "PASSED" : "NOT PASSED");
