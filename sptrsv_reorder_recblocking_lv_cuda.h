#ifndef _SPTRSV_REORDER_REBLOCKING_LV_CUDA_
#define _SPTRSV_REORDER_REBLOCKING_LV_CUDA_

#include "common.h"
#include "findlevel.h"
#include "utils_sptrsv_reorder_recblocking_cuda.h"
#include <cuda_runtime.h>

int sptrsv_reorder_recblocking_lv_cuda(const int           *cscColPtrTR,
                                    ...
                                    double        *preprocess)
{
    ...
    *preprocess += (t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0;
    ...
    // call normal recblocking SpTRSV
    sptrsv_reorder_recblocking_cuda(cscColPtrTR_new, cscRowIdxTR_new, cscValTR_new, m, n, nnzTR,
    ...
    *preprocess += (t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0;
    ...
    if (res < accuracy)
        printf("sptrsv_reorder_reblocking_lv_cuda: x check passed! |x-xref|/|xref| = %8.2e\n", res);
    else
        printf("sptrsv_reorder_reblocking_lv_cuda: x check _NOT_ passed! |x-xref|/|xref| = %8.2e\n", res);
